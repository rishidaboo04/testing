...
async def ask_question(request: QuestionRequest):
    try:
        start_time = time.time()  # Changed from asyncio.get_event_loop().time()
        ...
        response = rag_pipeline.run(  # Changed from run_async to run
            {
        ...
        execution_time = time.time() - start_time  # Changed timing calculation
        
        # Update response handling for the synchronous pipeline
        retrieved_docs_info = [
            DocumentInfo(
                id=doc.id,
                content=doc.content,
                meta={
                    ...
                    "line_number": doc.meta.get("line_number", 1),
        ...
        return RAGResponse(
            answer=response['llm']['generated_text'],  # Changed from ['replies'][0].text
            retrieved_documents=retrieved_docs_info,
            execution_time=execution_time
        )
        
    except Exception as e:
        print(f"Error details: {str(e)}")  # Add debugging information
